...
(function() {
  var getHash, getParameter, prependProtocolIfMissing, resizeit, scaleIframe, stripProtocolIfPresent;

  scaleIframe = function(iframeSelector, fixed_width, org_width, org_height) {
    var $el, _new_height, _scale;
    $el = $(iframeSelector);
    _scale = org_width / fixed_width;
    _new_height = org_height * 1 / _scale;
    return $el.css({
      "width": "" + fixed_width + "px",
      "height": "" + _new_height + "px",
  ...
  prependProtocolIfMissing = function(url) {
  ...
  stripProtocolIfPresent = function(url) {
    return (url && !url.match(/^http([s]?):\/\/.*/) ? url : url.substring(7)).split("#")[0];
  };
  ...
  resizeit = function() {
    ...
    return $("#wrapper").scrollTo($.data(document, "page"));
  };

  $(document).ready(function() {
    var clip, ipad_org_height, ipad_org_width, iphone_org_height, iphone_org_width, l, lang, _fixed_width, _footer_height_down, _footer_height_up, _url, _wrapper;
    l = getParameter("l");
    if (l === "de") {
      $("<input>").attr({
        type: "hidden",
    ...
    _wrapper = $("#wrapper");
    $("a.navaction").click(function() {
      var _arrow, _page, _that;
      _page = $(this).attr("href");
      _that = this;
      $.data(document, "page", _page);
      _arrow = (_page === "#ipad" ? $("#left-arrow") : $("#right-arrow"));
      $(".arrow").fadeOut();
      _wrapper.scrollTo(_page, 800, {
        onAfter: function() {
          _arrow.fadeIn();
          return $("html, body").animate({
            scrollTop: 0
          }, "fast", function() {
            return _wrapper.height($(_that).data("height"));
          });
        }
      });
      return false;
    });
    iphone_org_width = $(".iphone > iframe").width();
    iphone_org_height = $(".iphone > iframe").height();
    ipad_org_width = $(".ipad > iframe").width();
    ipad_org_height = $(".ipad > iframe").height();
    $("#width-slider").change(function() {
      var newVal;
      newVal = $("#width-slider").val();
      scaleIframe(".iphone > iframe", newVal, iphone_org_width, iphone_org_height);
      scaleIframe(".ipad > iframe", newVal, ipad_org_width, ipad_org_height);
      location.hash = newVal;
      $("#copy-me").val(document.location.href);
      return false;
    });
    $(window).resize(function() {
      return resizeit();
    });
    $.data(document, "page", "#iphone");
    resizeit();
    _url = prependProtocolIfMissing(decodeURIComponent(getParameter("url") || "http://www.flexify.net"));
    $("iframe").attr("src", _url);
    if ((_fixed_width = getHash() || false)) {
      console.log(_fixed_width);
      scaleIframe(".iphone > iframe", _fixed_width, iphone_org_width, iphone_org_height);
      scaleIframe(".ipad > iframe", _fixed_width, ipad_org_width, ipad_org_height);
      $("#width-slider").val(_fixed_width);
    }
    $("#copy-me").val(document.location.href);
    $("#copy-me").click(function() {
      return this.select();
    });
    $("input[name=\"url\"]").val(stripProtocolIfPresent(_url));
    clip = new ZeroClipboard($("#copy-button"), {
      moviePath: "ZeroClipboard.swf"
    });
    clip.on("complete", function(client, args) {
      var _old_text, _t_copying;
      _old_text = $("#copy-button").text();
      _t_copying = $("#copy-button").text("...").attr("disabled", true);
      return setTimeout(function() {
        $("#copy-button").text("OK!");
        return setTimeout(function() {
          return $("#copy-button").text(_old_text).removeAttr("disabled");
        }, 600);
      }, 700);
    });
    clip.setText(document.location.href);
    clip.glue($('#copy-button'), $('#copy-container'));
    _footer_height_up = 140;
    _footer_height_down = 30;
    return $("#footer-toggle").toggle(function() {
      $(".footer").animate({
        height: _footer_height_up
      });
      return clip.reposition();
    }, function() {
      $(".footer").animate({
        height: _footer_height_down
      });
      return clip.reposition();
    });
  });
